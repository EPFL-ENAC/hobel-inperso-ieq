...
def compute_light_percent(datetime_start: datetime, datetime_end: datetime, df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_minute["threshold"] = df_minute["hour"].apply(
        lambda h: day_threshold if day_start_hour <= h < night_start_hour else night_threshold
    )
    df_minute["above_threshold"] = df_minute["value"] > df_minute["threshold"]

    df_light_percent = (
        df_minute.groupby(["device", "hour"])
    ...
    df_light_percent["field"] = "light_percent"
